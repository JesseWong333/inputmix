...
class Attention(nn.Module):
    ...
    def cal_qkv(self, x): #  added by junjie
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # q: B * N_heads * T * C
        return q, k, v
    ...
    def attend_with_summary_info(self, q, k, v, summary_k, summaried_v):  #  added by junjie
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, T, N_heads*C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, attn
...
class VisionTransformer(nn.Module):
    ...
    def get_selfattention(self, x, n=1):
        """
        added by Junjie Wang
        # return the last n-th attention
        """
        x = self.prepare_tokens(x)
        for i, blk in enumerate(self.blocks):
